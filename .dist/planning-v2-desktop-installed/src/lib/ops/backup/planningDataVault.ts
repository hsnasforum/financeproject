...
import { createRun, getRun, getRunBlob, hardDeleteRun } from "../../planning/server/store/runStore";
import { hardDeleteProfile, listProfiles, upsertProfileRecord } from "../../planning/store/profileStore";
import { resolveRunBlobsDir, resolveRunDir, resolveRunsDir, resolveRunsIndexPath, sanitizeRecordId } from "../../planning/store/paths";
...
export type VaultPreviewSummary = {
  manifest: Pick<VaultManifest, "kind" | "formatVersion" | "createdAt" | "appVersion" | "schemaVersions" | "counts" | "latestSnapshotId">;
  actual: {
    totalFiles: number;
    totalBytes: number;
    profiles: number;
...
async function collectRunIds(): Promise<string[]> {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      try {
        ids.add(sanitizeRecordId(entry.name));
      } catch {
    ...
    try {
      ids.add(sanitizeRecordId(entry.name.slice(0, -5)));
    } catch {
      // ignore unknown file
    }
  }
...
async function collectRunBlobBuffers(runId: string): Promise<Map<string, Buffer>> {
  const out = new Map<string, Buffer>();
  const blobsDir = resolveRunBlobsDir(runId);
  let entries: Awaited<ReturnType<typeof fs.readdir>>;
  try {
...
function toManifestWithoutFiles(manifest: VaultManifest): Pick<VaultManifest, "kind" | "formatVersion" | "createdAt" | "appVersion" | "schemaVersions" | "counts" | "latestSnapshotId"> {
  return {
    kind: manifest.kind,
...
export async function buildPlanningDataVaultZip(): Promise<{
  fileName: string;
  bytes: Buffer;
  manifest: VaultManifest;
}> {
  ...
  for (const runId of runIds) {
    ...
    const blobs = await collectRunBlobBuffers(runId);
    for (const [blobName, blobBytes] of blobs) {
      if (!blobName) continue;
      fileEntries.set(runBlobPath(runId, blobName), blobBytes);
    }

    const planBytes = await readStoredJsonBuffer(path.join(resolveRunDir(runId), "action-plan.json"));
    if (planBytes) {
      fileEntries.set(runActionPlanPath(runId), planBytes);
    }
    const progressBytes = await readStoredJsonBuffer(path.join(resolveRunDir(runId), "action-progress.json"));
    if (progressBytes) {
  ...
  let historyEntries: Awaited<ReturnType<typeof fs.readdir>>;
  try {
  ...
  const policyBytes = await readFileIfExists(path.join(process.cwd(), OPS_AUTO_MERGE_POLICY_FILE));
  if (policyBytes) {
  ...
  return {
    fileName: `planning-data-vault-${stamp}.zip`,
    bytes: zipBuffer,
    manifest,
  };
}
  ...
  for (const entryPath of profilePaths) {
    ...
    try {
      ...
      await upsertProfileRecord({
        id,
        name,
        profile: profilePayload as Parameters<typeof upsertProfileRecord>[0]["profile"],
    ...
    try {
      ...
      if (planBytes) {
        try {
          ...
          await writeStoreJsonAtomic(path.join(resolveRunDir(safeRunId), "action-plan.json"), plan);
      ...
      if (progressBytes) {
        try {
          ...
          await writeStoreJsonAtomic(path.join(resolveRunDir(safeRunId), "action-progress.json"), progress);
