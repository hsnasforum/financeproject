...
const PROFILE_META_PATH = ".data/planning/vault/profiles.meta.json";
...
function resolveProfileMetaPath(cwd = process.cwd()): string {
  ...
  return path.resolve(cwd, override || PROFILE_META_PATH);
}
...
async function resolveDefaultProfileId(records: PlanningProfileRecord[]): Promise<string | undefined> {
  ...
  const existing = asString(state.defaultProfileId);
  if (existing && records.some((row) => row.id === existing)) {
    return existing;
  }
