...
export async function GET(request: Request) {
  try {
    ...
    const result = await getFinlifeProducts("credit-loan", { pageNo, topFinGrpNo, scan, scanMaxPages });
    ...
    return NextResponse.json(
      { ok: false, error: { code: "INTERNAL", message: "FINLIFE 신용대출 API 처리 중 오류가 발생했습니다." } },
      { status: 503 },
    );
  }
}
