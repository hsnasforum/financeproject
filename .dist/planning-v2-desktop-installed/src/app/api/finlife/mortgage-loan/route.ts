...
export async function GET(request: Request) {
  try {
    ...
    const result = await getFinlifeProducts("mortgage-loan", { pageNo, topFinGrpNo, scan, scanMaxPages });
    ...
    return NextResponse.json(
      { ok: false, error: { code: "INTERNAL", message: "FINLIFE 주담대 API 처리 중 오류가 발생했습니다." } },
      { status: 503 },
    );
  }
}
