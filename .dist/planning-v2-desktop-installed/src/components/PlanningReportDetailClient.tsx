...
export default function PlanningReportDetailClient({ id }: Props) {
  ...
  const [advancedRawRunJson, setAdvancedRawRunJson] = useState<unknown>(null);
  ...
  useEffect(() => {
    ...
    async function loadAdvancedRawIfNeeded(): Promise<void> {
      ...
      try {
        ...
        const response = await fetch(`/api/planning/runs/${encodeURIComponent(runId)}/blob/raw${query}`, { cache: "no-store" });
        ...
        setAdvancedRawRunJson(payload.data ?? null);
      } catch (loadError) {
        if (!active) return;
    ...
    void loadAdvancedRawIfNeeded();
    return () => {
      active = false;
    };
  }, [advancedOpen, advancedRawLoading, advancedRawRunJson, report?.runId, resolvedRunId, run?.id]);
            ...
            <div className="space-y-2" id="planning-reports-advanced-panel">
              ...
              <ReportAdvancedRaw
                ...
                raw={{
                  ...
                  ...(advancedRawRunJson !== null ? { runJson: advancedRawRunJson } : {}),
                }}
              />
            </div>
          ) : null}
