    ...
    (async () => {
      try {
        ...
        if (!response.ok || !payload?.ok) {
          throw new Error(payload?.error?.message ?? "CSRF 토큰을 불러오지 못했습니다.");
        }
  ...
  async function handleExport(): Promise<void> {
    ...
    try {
      const response = await fetch("/api/ops/backup/export", {
        ...
        body: JSON.stringify({ csrf, passphrase }),
      });
      if (!response.ok) {
      ...
      setNotice(`암호화 백업 파일을 다운로드했습니다. (${fileName})`);
    } catch (exportError) {
      ...
      {preview ? (
        <Card className="mt-6" data-testid="ops-backup-preview-card">
          ...
          <p className="mt-2 text-xs text-slate-600">
            kind={preview.manifest?.kind ?? "-"} · formatVersion={preview.manifest?.formatVersion ?? "-"} · appVersion={preview.manifest?.appVersion ?? "-"} · createdAt={formatDateTime(preview.manifest?.createdAt)}
          </p>
