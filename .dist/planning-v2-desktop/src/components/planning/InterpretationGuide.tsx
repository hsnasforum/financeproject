...
export default function InterpretationGuide(props: InterpretationGuideProps) {
  const vm = buildInterpretationVM(
    ...
    props.policy ?? DEFAULT_PLANNING_POLICY,
  );
  const badge = verdictBadge(vm.verdict.code);
  const className = props.className ? ` ${props.className}` : "";

  return (
    <Card className={`space-y-3 border border-slate-200 p-4${className}`} data-testid="interpretation-guide">
        ...
        {vm.diagnostics.length === 0 ? (
          ...
          <div className="mt-2 space-y-2">
            {vm.diagnostics.slice(0, 3).map((diag) => (
              <article className={`rounded-lg border px-3 py-2 text-xs ${diagnosticTone(diag.severity)}`} key={diag.id}>
            ...
            <article className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" key={`${action.id}-${index}`}>
              ...
              {action.href ? (
                <p className="mt-2 text-[11px] text-slate-600">
                  <Link className="font-semibold underline" href={action.href}>{action.href}</Link>
                </p>
              ) : null}
            </article>
          ))}
