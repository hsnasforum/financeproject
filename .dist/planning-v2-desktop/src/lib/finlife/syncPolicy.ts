export function shouldRunFinlifeSync(
  snapshotMeta: { generatedAt?: string; completionRate?: number } | null | undefined,
  nowMs: number,
  opts?: { ttlMs?: number; minCompletionRate?: number },
): { shouldRun: boolean; reason: "missing_snapshot" | "stale_snapshot" | "low_completion_rate" | "fresh" } {
  const ttlMs = Math.max(60_000, opts?.ttlMs ?? 12 * 60 * 60 * 1000);
  ...
  const generatedMs = Date.parse(snapshotMeta.generatedAt ?? "");
  ...
  if (completionRate < minCompletionRate) return { shouldRun: true, reason: "low_completion_rate" };
