...
export async function POST(request: Request, context: { params: Promise<Params> }) {
  ...
  if (!verifyConfirm(confirmText, expectedConfirm)) {
    ...
    return jsonError("CONFIRM_MISMATCH", message, { status: 400, meta: { expectedConfirm } });
  }
  ...
  if (!feedback) {
    ...
    return jsonError("NO_DATA", message, { status: 404 });
  }
  ...
  if (feedback.triage.status === "new") {
    ...
    return jsonError("NOT_TRIAGED", message, { status: 400 });
  }
  ...
  if (feedback.link?.githubIssue?.url) {
    ...
    return jsonError("ALREADY_LINKED", message, { status: 409 });
  }
  ...
  try {
    ...
    if (!updated) {
      ...
      return jsonError("STORE_WRITE_FAILED", message, { status: 500 });
    }
    ...
    return jsonError("GITHUB_ISSUE_FAILED", message, { status: 502 });
  }
}
