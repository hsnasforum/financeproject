...
describe("run action progress route", () => {
  ...
  it("persists progress updates and enforces csrf/local-only guards", async () => {
    ...
    const remotePayload = await remoteRes.json() as { error?: { code?: string } };
    expect(remoteRes.status).toBe(403);
    expect(remotePayload.error?.code).toBe("LOCAL_ONLY");

    const remotePatchRes = await actionProgressPATCH(
      new Request(`${REMOTE_ORIGIN}/api/planning/runs/${runId}/action-progress`, {
        method: "PATCH",
        headers: {
          host: REMOTE_HOST,
    ...
    const remotePatchPayload = await remotePatchRes.json() as { error?: { code?: string } };
