...
describe("POST /api/planning/v3/draft/apply", () => {
  ...
  it("apply mode saves recomputed patch values to the target profile", async () => {
    const profile = await createProfile({
      name: "Apply Target",
      profile: {
    ...
    expect(payload.ok).toBe(true);
    expect(payload.savedProfileId).toBe(profile.id);
    expect(typeof payload.savedAt).toBe("string");

    const saved = await getProfile(profile.id);
    expect(saved).not.toBeNull();
    expect(saved?.profile.monthlyIncomeNet).toBe(3_100_000);
    expect(saved?.profile.monthlyEssentialExpenses).toBe(1_000_000);
    expect(saved?.profile.monthlyDiscretionaryExpenses).toBe(0);
  });
