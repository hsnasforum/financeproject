import { expect, test } from "@playwright/test";

test("planning v3 import page shows meta and cashflow table", async ({ page }) => {
  const csv = [
    "date,amount,desc",
    "2026-01-01,1000000,salary",
    "2026-01-05,-300000,rent",
    "2026-02-01,1200000,salary",
  ].join("\n");

  await page.route("**/api/planning/v3/import/csv", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        ok: true,
        cashflow: [
          { ym: "2026-01", incomeKrw: 1_000_000, expenseKrw: -300_000, netKrw: 700_000, txCount: 2 },
          { ym: "2026-02", incomeKrw: 1_200_000, expenseKrw: 0, netKrw: 1_200_000, txCount: 1 },
        ],
        draftPatch: {
          monthlyIncomeNet: 950_000,
          monthlyEssentialExpenses: 210_000,
          monthlyDiscretionaryExpenses: 90_000,
        },
        meta: { rows: 3, months: 2 },
      }),
    });
  });

  await page.goto("/planning/v3/import");
  await page.getByTestId("v3-csv-text").fill(csv);
  await page.getByTestId("v3-csv-submit").click();
  await page.getByTestId("v3-mapping-apply").click();

  await expect(page.getByTestId("v3-import-meta")).toBeVisible();
  await expect(page.getByTestId("v3-cashflow-table")).toBeVisible();
});

test("planning v3 import can save draft and list appears in /planning/v3/drafts", async ({ page }) => {
  const csv = [
    "date,amount,desc",
    "2026-01-01,1000000,salary",
    "2026-01-05,-300000,rent",
    "2026-02-01,1200000,salary",
  ].join("\n");

  await page.route("**/api/planning/v3/import/csv", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        ok: true,
        cashflow: [
          { ym: "2026-01", incomeKrw: 1_000_000, expenseKrw: -300_000, netKrw: 700_000, txCount: 2 },
          { ym: "2026-02", incomeKrw: 1_200_000, expenseKrw: 0, netKrw: 1_200_000, txCount: 1 },
        ],
        draftPatch: {
          monthlyIncomeNet: 950_000,
          monthlyEssentialExpenses: 210_000,
          monthlyDiscretionaryExpenses: 90_000,
          assumptions: [
            "monthlyIncomeNet uses median monthly net (assumption)",
            "expense split 70/30 (assumption)",
          ],
          monthsConsidered: 2,
        },
        meta: { rows: 3, months: 2 },
      }),
    });
  });

  await page.goto("/planning/v3/drafts");
  const beforeCount = await page.getByTestId("v3-draft-row").count();

  await page.goto("/planning/v3/import");
  await page.getByTestId("v3-csv-text").fill(csv);
  await page.getByTestId("v3-csv-submit").click();
  await page.getByTestId("v3-mapping-apply").click();
  await page.getByTestId("v3-save-draft").click();
  await expect(page.getByRole("link", { name: "초안 목록 보기" })).toBeVisible();
  await page.getByRole("link", { name: "초안 목록 보기" }).click();

  await expect(page).toHaveURL(/\/planning\/v3\/drafts$/);
  await expect(page.getByTestId("v3-drafts-table")).toBeVisible();

  const afterCount = await page.getByTestId("v3-draft-row").count();
  expect(afterCount).toBeGreaterThan(beforeCount);
});
