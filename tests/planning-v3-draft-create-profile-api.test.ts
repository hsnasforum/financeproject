...
describe("POST /api/planning/v3/drafts/[id]/create-profile", () => {
  ...
  it("creates a new profile from draft apply result", async () => {
    ...
    const payload = await response.json() as { ok?: boolean; profileId?: string };
    expect(payload.ok).toBe(true);
    expect(payload.profileId).toBeTruthy();

    const metas = await listProfileMetas();
    expect(metas.some((row) => row.profileId === payload.profileId)).toBe(true);
  });
});
