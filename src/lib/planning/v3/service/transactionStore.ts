...
export type MergeBatchesResult = {
  mergedCount: number;
  dedupedCount: number;
};

export type UpdateBatchAccountResult = {
  batch: V3ImportBatch;
...
function normalizeAccountId(value: unknown): string {
  ...
  if (!text) {
    throw new TransactionStoreInputError("accountId is required", [
      { field: "accountId", message: "거래 계좌를 선택해 주세요." },
    ]);
  }
  try {
    return sanitizeRecordId(text);
  } catch {
    throw new TransactionStoreInputError("invalid accountId", [
      { field: "accountId", message: "거래 계좌 형식이 올바르지 않습니다." },
    ]);
  }
}
...
function normalizeImportBatch(value: unknown): V3ImportBatch | null {
  ...
  try {
    ...
    return {
      ...
      ...(asString(value.accountId) ? { accountId: sanitizeRecordId(asString(value.accountId)) } : {}),
      ...(normalizeFileName(value.accountHint) ? { accountHint: normalizeFileName(value.accountHint) } : {}),
...
function normalizeTransactionRecord(value: unknown): V3TransactionRecord | null {
  ...
  try {
    ...
    const txnId = isTxnId(value.txnId)
      ? value.txnId
      : buildTxnId({
          dateIso: date,
          amountKrw,
...
async function readBatches(): Promise<V3ImportBatch[]> {
  ...
  const byId = new Map<string, V3ImportBatch>();
  for (const row of rows) {
    const normalized = normalizeImportBatch(row);
...
function createStableBatchId(csvSha256: string, mapping: CsvColumnMapping, accountId: string): string {
  ...
  const base = `b_${toSha256(`${csvSha256}:${mappingSignature}:${accountId}`).slice(0, 24)}`;
  return sanitizeRecordId(base);
}

function summarizeMonths(records: Array<Pick<AccountTransaction, "date" | "amountKrw">>): Array<{
  ym: string;
  incomeKrw: number;
  expenseKrw: number;
  netKrw: number;
...
function summarizeAccountMonthlyNet(records: AccountTransaction[]): Array<{
  accountId: string;
  ym: string;
  netKrw: number;
  txCount: number;
  ...
  for (const record of records) {
    ...
    const ym = asString(record.date).slice(0, 7);
...
function compareTransactionsByTxnId(left: AccountTransaction, right: AccountTransaction): number {
...
function allocateRecordId(batchId: string, txnId: string, usedIds: Set<string>): string {
  const base = sanitizeRecordId(`t_${toSha256(`${batchId}:${txnId}`).slice(0, 24)}`);
  if (!usedIds.has(base)) {
    usedIds.add(base);
    return base;
  }
  ...
  for (let seq = 1; seq < 10_000; seq += 1) {
    const candidate = sanitizeRecordId(`${base}_${seq}`);
  ...
  throw new Error("failed to allocate transaction record id");
}

function toAccountTransaction(record: V3TransactionRecord, fallbackAccountId?: string): AccountTransaction {
  const accountId = asString(record.accountId) || asString(fallbackAccountId);
  return {
    txnId: record.txnId,
    ...(accountId ? { accountId } : {}),
    date: record.date,
...
function toBatchDetailSampleFromTransaction(tx: AccountTransaction): CsvPreviewRow {
  return {
    line: tx.meta?.rowIndex ?? 0,
    dateIso: tx.date,
    amountKrw: tx.amountKrw,
    ...(tx.description ? { descMasked: tx.description } : {}),
    ok: true,
  };
}

function toRecordFromTransaction(input: {
  ...
  const lineNo = input.tx.meta?.rowIndex ?? 0;
  const descMasked = maskPreviewDescription(input.tx.description);
  const accountId = asString(input.accountId);
  const txnId = isTxnId(input.tx.txnId)
    ? input.tx.txnId
    : buildTxnId({
        dateIso: input.tx.date,
        amountKrw: input.tx.amountKrw,
        descNorm: normalizeDescriptionForTxnId(input.tx.description),
        ...(accountId ? { accountId } : {}),
      });

  const id = allocateRecordId(input.batchId, txnId, input.usedIds);
  return {
    id,
    txnId,
    ...
    sourceInfo: {
      ...
      ...(asString(input.csvSha256) ? { sha256: asString(input.csvSha256) } : {}),
    },
  };
}

function mergeBatchMeta(existing: V3ImportBatch | null, incoming: V3ImportBatch, addedStoredCount: number): V3ImportBatch {
  ...
  return {
    ...
    ...(incoming.accountHint ? { accountHint: incoming.accountHint } : {}),
    ...(incoming.sha256 ? { sha256: incoming.sha256 } : {}),
    total: Math.max(existing.total, incoming.total),
    ok: existing.ok + addedStoredCount,
    failed: Math.max(existing.failed, incoming.failed),
  };
}
...
export async function appendBatchFromCsv(input: AppendBatchFromCsvInput): Promise<AppendBatchFromCsvResult> {
  ...
  const batchId = createStableBatchId(csvSha256, mappingUsed, accountId);
  const dedupedParsed = dedupeTransactions(parsed.transactions);

  const existingBatches = await readBatches();
  const existingBatch = existingBatches.find((entry) => entry.id === batchId) ?? null;
  const existingRecords = await readTransactionRecords();
  const usedRecordIds = new Set(existingRecords.map((record) => record.id));
  const existingTxnIds = new Set(existingRecords.map((record) => record.txnId));

  const newTransactions = dedupedParsed.transactions.filter((tx) => {
    const txnId = asString(tx.txnId);
    return txnId.length > 0 && !existingTxnIds.has(txnId);
  });

  const dedupedAgainstStore = dedupedParsed.transactions.length - newTransactions.length;
  const totalDeduped = dedupedParsed.dedupedCount + dedupedAgainstStore;

  const incomingBatch: V3ImportBatch = {
    id: batchId,
    createdAt,
    kind: "csv",
    ...(fileName ? { fileName } : {}),
    ...
    ok: newTransactions.length,
    failed: parsed.stats.skipped,
  };
  const mergedBatch = mergeBatchMeta(existingBatch, incomingBatch, newTransactions.length);

  if (!existingBatch || newTransactions.length > 0) {
    await appendNdjsonLine(resolveBatchesPath(), mergedBatch);
  }

  for (const tx of newTransactions) {
    const record = toRecordFromTransaction({
      tx,
      batchId,
      createdAt,
      ...(fileName ? { fileName } : {}),
  ...
  const batch = existingBatch && newTransactions.length < 1
    ? existingBatch
    : mergedBatch;
...
export async function readBatch(batchId: string): Promise<ReadBatchResult | null> {
  ...
  const deduped = dedupeTransactions(records.map((record) => toAccountTransaction(record, batch.accountId)));
  const transactions = deduped.transactions.sort(compareTransactionsByTxnId);
  const sample = transactions.slice(0, 20).map((row) => toBatchDetailSampleFromTransaction(row));
  const monthsSummary = summarizeMonths(transactions);
  const accountMonthlyNet = summarizeAccountMonthlyNet(transactions);
...
export async function readBatchTransactions(batchId: string): Promise<ReadBatchTransactionsResult | null> {
  ...
  const records = (await readTransactionRecords())
    .filter((record) => record.batchId === safeBatchId)
    .map((record) => toAccountTransaction(record, batch.accountId));
  const transactions = dedupeTransactions(records).transactions.sort(compareTransactionsByTxnId);
...
export async function readAllTransactions(): Promise<AccountTransaction[]> {
  ...
  const all = (await readTransactionRecords())
    .map((record) => toAccountTransaction(record, batchById.get(record.batchId)?.accountId))
    .sort(compareTransactionsByTxnId);
  return dedupeTransactions(all).transactions.sort(compareTransactionsByTxnId);
}

export async function updateBatchAccount(batchId: string, accountId: string): Promise<UpdateBatchAccountResult | null> {
  assertServerOnly();
  ...
  const updated: V3ImportBatch = {
    ...batch,
    createdAt: nowIso(),
    accountId: safeAccountId,
    accountHint: safeAccountId,
  };
  await appendNdjsonLine(resolveBatchesPath(), updated);

  const changedRecords = (await readTransactionRecords())
    .filter((record) => record.batchId === safeBatchId && asString(record.accountId) !== safeAccountId)
    .length;
...
export async function mergeBatches(fromBatchId: string, intoBatchId: string): Promise<MergeBatchesResult> {
  ...
  const safeFromBatchId = sanitizeRecordId(fromBatchId);
  const safeIntoBatchId = sanitizeRecordId(intoBatchId);
  if (safeFromBatchId === safeIntoBatchId) {
    throw new TransactionStoreInputError("invalid merge target", [
      { field: "batchId", message: "서로 다른 배치를 선택해 주세요." },
    ]);
  }
  ...
  const fromBatch = batches.find((entry) => entry.id === safeFromBatchId) ?? null;
  const intoBatch = batches.find((entry) => entry.id === safeIntoBatchId) ?? null;
  if (!fromBatch || !intoBatch) {
    throw new TransactionStoreInputError("batch not found", [
      { field: "batchId", message: "병합할 배치를 찾을 수 없습니다." },
    ]);
  }

  const fromTransactions = dedupeTransactions(
    records
      .filter((record) => record.batchId === safeFromBatchId)
      .map((record) => toAccountTransaction(record, fromBatch.accountId)),
  ).transactions;

  const intoTransactions = dedupeTransactions(
    records
      .filter((record) => record.batchId === safeIntoBatchId)
      .map((record) => toAccountTransaction(record, intoBatch.accountId)),
  ).transactions;

  const intoTxnIds = new Set(intoTransactions.map((tx) => asString(tx.txnId)).filter((txnId) => txnId.length > 0));
  const mergeCandidates = fromTransactions.filter((tx) => {
    const txnId = asString(tx.txnId);
    return txnId.length > 0 && !intoTxnIds.has(txnId);
  });

  const dedupedCount = Math.max(0, fromTransactions.length - mergeCandidates.length);
  ...
  for (const tx of mergeCandidates) {
    const record = toRecordFromTransaction({
      tx,
      batchId: safeIntoBatchId,
      createdAt,
      ...(intoBatch.fileName ? { fileName: intoBatch.fileName } : {}),
      csvSha256: asString(intoBatch.sha256) || asString(fromBatch.sha256),
      accountId: asString(tx.accountId) || asString(intoBatch.accountId) || asString(fromBatch.accountId),
      usedIds,
    });
    await appendNdjsonLine(resolveRecordsPath(), record);
  }

  const mergedBatch: V3ImportBatch = {
    ...intoBatch,
    createdAt,
    ok: intoBatch.ok + mergeCandidates.length,
    ...(intoBatch.accountId ? { accountId: intoBatch.accountId } : {}),
  };
  await appendNdjsonLine(resolveBatchesPath(), mergedBatch);
