...
export async function GET(request: Request) {
  ...
  const workflow = asString(searchParams.get("workflow"));
  const ref = asString(searchParams.get("ref"));
  const since = asString(searchParams.get("since"));
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "RUNS_FAILED",
          message: error instanceof Error ? error.message : "runs 조회에 실패했습니다.",
        },
      },
      { status: 502 },
    );
