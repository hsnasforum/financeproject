...
function parseNormalizationDisclosure(value: unknown): ProfileNormalizationDisclosure | undefined {
  ...
  if (defaultsApplied.length < 1 && fixesApplied.length < 1) return undefined;
  return {
    defaultsApplied,
    fixesApplied,
  };
...
function summaryMonthlySurplusKrw(dto: ResultDtoV1): { value?: number; evidence?: CalcEvidence } {
  ...
  if (typeof income !== "number" || typeof expenses !== "number") return {};
  return {
    value: computeMonthlySurplusKrw(income, expenses, debtPayment),
    evidence: buildMonthlySurplusEvidence({
      monthlyIncomeKrw: income,
      monthlyExpensesKrw: expenses,
      monthlyDebtPaymentKrw: debtPayment,
    }),
  };
}

function summaryEmergencyFundMonths(dto: ResultDtoV1): { value?: number; evidence?: CalcEvidence } {
  ...
  if (typeof currentKrw !== "number") return {};
  const start = dto.timeline.points.find((point) => point.label === "start") ?? dto.timeline.points[0];
  const monthlyExpense = asNumber(start?.expensesKrw);
  if (typeof monthlyExpense !== "number" || monthlyExpense <= 0) return {};
  const months = computeEmergencyFundMonths(currentKrw, monthlyExpense);
  if (typeof months !== "number") return {};
  return {
    value: months,
    evidence: buildEmergencyMonthsEvidence({
      emergencyFundKrw: currentKrw,
      monthlyExpensesKrw: monthlyExpense,
    }),
  };
}
...
function summaryDsrPct(
  dto: ResultDtoV1,
  debtSummary: ReportVM["debtSummary"] | undefined,
): { value?: number; evidence?: CalcEvidence } {
  const start = dto.timeline.points.find((point) => point.label === "start") ?? dto.timeline.points[0];
  const income = asNumber(start?.incomeKrw);
  const debtPayment = typeof debtSummary?.meta.totalMonthlyPaymentKrw === "number"
    ? debtSummary.meta.totalMonthlyPaymentKrw
    : asNumber(start?.debtPaymentKrw);
  if (typeof income !== "number" || typeof debtPayment !== "number") return {};
  return {
    value: computeDsrPct(debtPayment, income),
    evidence: buildDsrPctEvidence({
      monthlyDebtPaymentKrw: debtPayment,
      monthlyIncomeKrw: income,
    }),
  };
...
export function buildReportVM(
  ...
  const monthlySurplus = summaryMonthlySurplusKrw(dto);
  const monthlySurplusKrw = monthlySurplus.value;
  const emergencyFund = summaryEmergencyFundMonths(dto);
  const emergencyFundMonths = emergencyFund.value;
  const debtTotalKrw = summaryDebtTotalKrw(dto);
  const totalMonthlyDebtPaymentKrw = debtSummary?.meta.totalMonthlyPaymentKrw;
  const dsr = summaryDsrPct(dto, debtSummary);
  const dtoDsrPct = asNumber(dto.summary.dsrPct);
  const dsrPct = typeof dtoDsrPct === "number" ? dtoDsrPct : dsr.value;
  const summaryEvidence: ReportSummaryEvidence = {
    ...(monthlySurplus.evidence ? { monthlySurplusKrw: monthlySurplus.evidence } : {}),
    ...(emergencyFund.evidence ? { emergencyFundMonths: emergencyFund.evidence } : {}),
    ...(dsr.evidence ? { dsrPct: dsr.evidence } : {}),
  };
