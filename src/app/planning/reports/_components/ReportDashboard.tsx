...
export default function ReportDashboard({ vm }: Props) {
  ...
  return (
    <div className="space-y-5" data-testid="report-dashboard">
        ...
        <Card className="space-y-3 p-5" data-testid="report-summary-cards">
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              {evidenceById.get("monthlySurplus") ? (
                <EvidencePanel className="mt-2" item={evidenceById.get("monthlySurplus")!} />
              ) : null}
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              {evidenceById.get("dsrPct") ? (
                <EvidencePanel className="mt-2" item={evidenceById.get("dsrPct")!} />
              ) : null}
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              {evidenceById.get("emergency") ? (
                <EvidencePanel className="mt-2" item={evidenceById.get("emergency")!} />
              ) : null}
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
      ...
      {hasNormalization ? (
        <Card className="space-y-2 p-5" data-testid="report-normalization-disclosure">
          <details className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <summary className="cursor-pointer text-sm font-semibold text-slate-800">
              자동 보정/기본값 적용 내역
            </summary>
            {normalization?.defaultsApplied.length ? (
              <div className="mt-2">
                <p className="text-xs font-semibold text-slate-700">defaultsApplied</p>
            ...
            {normalization?.fixesApplied.length ? (
              <div className="mt-2">
                <p className="text-xs font-semibold text-slate-700">fixesApplied</p>
                <ul className="mt-1 list-disc space-y-1 pl-4 text-xs text-slate-700">
