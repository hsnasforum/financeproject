...
## 현재 상태
- 정책 정리 시점 기준 `analysis_docs/**`는 Git 기준 untracked 상태였습니다.
...
- 이번 라운드는 운영 정책 정리만 수행하며, `git add`, tracking 전환, rename, delete, 본문 대량 수정은 하지 않습니다.

## Git 추적 추천안
- [권장안] `analysis_docs/**`는 별도 운영 라운드에서 Git 추적 대상으로 전환합니다.
...
- 이유 4: 이번 라운드 범위는 정책 문서화뿐이므로, 실제 `git add`와 추적 전환은 문서 세트 범위를 확정한 뒤 한 번에 처리하는 편이 안전합니다.
- [보류] 실제 추적 전환 시점에는 `analysis_docs/**` 전체를 한 배치로 올릴지, `README + 5종 본문`만 먼저 올릴지 범위를 한 번 더 정해야 합니다.
- [추가 결정 필요] 추적 전환 라운드에서 오늘 생성된 `analysis-docs-*` `/work` 메모를 모두 같이 올릴지, 대표본만 먼저 올릴지 별도 결정이 필요합니다.
...
## 남은 결정 사항
- [추가 결정 필요] 실제 Git 추적 전환을 어느 운영 라운드에서 할지
- [추가 결정 필요] 추적 전환 시 `analysis_docs/**` 전체와 관련 `/work` 대표본을 같은 배치로 묶을지
