...
세부 결정:
- canonical field name은 `[권장안] recommendRunId`로 고정합니다.
- reverse ref owner 위치는 `PlanningRunRecord` core schema가 아니라 planning canonical owner를 해치지 않는 얇은 report-side metadata / read-model projection으로 제한합니다.
- canonical reverse href는 `/recommend/history?open=<recommendRunId>`입니다.
- explicit ref는 report UI에서 먼저 읽고, export는 같은 ref를 summary 수준으로만 붙이는 후속 순서로 둡니다.

메모:
- 이번 라운드에서는 reverse ref 저장 자체를 열지 않으므로 문서 제안까지만 남깁니다.
- report/export는 `recommendRunId`가 없으면 reverse link를 숨기고, planning run id로 fallback하지 않습니다.
- reverse link는 planning report 기준으로만 열고, legacy `/report` builder에는 같은 규칙을 적용하지 않습니다.

금지 규칙:
- `latest-match`로 가장 최근 recommend history를 붙이지 않습니다.
- 저장 시각, 시간대, profile 유사도 같은 heuristic으로 reverse ref를 추정하지 않습니다.
- `PlanningRunRecord.id`, `SavedRecommendRun.runId`, `SavedRunProfile.planning.runId`를 서로 대체하지 않습니다.
...
- 후보 owner: report UI와 export가 함께 읽을 수 있는 planning report-side explicit `recommendRunId`
...
- export에는 reverse link UI가 안정된 뒤 같은 `recommendRunId`를 요약 메모 수준으로만 붙임
...
- reverse link explicit ref는 `[권장안] recommendRunId`로 고정하고, planning report-side metadata에서만 먼저 읽도록 정합니다.
- reverse link first path는 planning report UI에서 `recommendRunId`가 있을 때만 `/recommend/history?open=<recommendRunId>`를 여는 경로를 우선합니다.
